...
def fly_line(startpos: np.array, targetpos: np.array, frames: int):
    x0, y0, z0 = startpos[:]
    x1, y1, z1 = targetpos[:]
    output = np.zeros((frames, 3))

    for i in range(frames):

        k = (i+1)/(frames)

        output[i, :] = x0 + (x1-x0)*k, y0 + (y1-y0)*k, z0 + (z1-z0)*k
...
def run(
    ...
    POS1 = np.array([0, 1, 1.5])
    POS2 = np.array([1, 0.5, 1])
    ...
    TARGET_POS[:step] = fly_line(INIT_XYZS[0], POS1, step)
    TARGET_POS[step:2 * step] = fly_line(POS1, POS2, step)
    TARGET_POS[2 * step:] = fly_line(POS2, INIT_XYZS[0], step)


    # таким образом можно склеить разные ломаные
